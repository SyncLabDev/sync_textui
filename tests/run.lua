...
test('new upsert suppresses ox_lib TextUI; update does not', function()
    Sync.State.Reset()
    local calls = 0
    Sync.Bridge = { SuppressForeignTextUI = function() calls = calls + 1 end }
    Sync.State.Upsert(Sync.Validation.NormalizeInteraction({ id = 'ox', text = 'A' }))
    eq(calls, 1, 'exactly once on fresh show')
    Sync.State.Upsert(Sync.Validation.NormalizeInteraction({ id = 'ox', text = 'B' }))
    eq(calls, 1, 'not called again for existing id')
    Sync.Bridge = nil
end)
...
test('partial updates do not clobber channel/priority/defaults', function()
    Sync.State.Reset()
    Sync.State.Upsert(Sync.Validation.NormalizeInteraction({ id = 'pc', text = 'A', description = 'B', channel = 'vehicle', priority = 7 }))
    truthy(Sync.State.ApplyChanges('pc', { progress = 40 }))
    local cur = Sync.State.GetVisible()[1]
    eq(cur.channel, 'vehicle', 'channel survives SetProgress-style update')
    eq(cur.priority, 7, 'priority survives SetProgress-style update')
    eq(cur.description, 'B', 'description survives')
    eq(cur.text, 'A', 'text survives')
    eq(cur.progress, 40)
    truthy(Sync.State.ApplyChanges('pc', { state = 'success' }))
    cur = Sync.State.GetVisible()[1]
    eq(cur.state, 'success')
    eq(cur.channel, 'vehicle', 'channel survives SetState')
    eq(cur.priority, 7, 'priority survives SetState')
end)
